...
const Movie = Converter(
  ({ id, imageUrl, title, rating, genres, releaseYear, duration, description }, context) => ({
    ...
    vote_average: rating,
    genres,
    genre_ids: genres?.map((item) => Genre.using(context).convert(item)).filter((item) => item ?? null),
    release_date: ReleaseYear.convert(releaseYear),
    runtime: duration,
    overview: description,
  }),
  ({ id, poster_path, title, vote_average, genre_ids, genres, release_date, runtime, overview }, context) => ({
    id,
